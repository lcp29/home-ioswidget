...
class HomeStore {
    static var shared: HomeStore = HomeStore()
    var homeInfos: [UUID: HomeInfo] = [:]
    var selected: HomeInfo? = nil
    ...
    private static func fileURL() throws -> URL {
        ...
        guard var url = FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier: "group.com.helmholtz.haassistant") else {
            throw NSError(domain: "", code: 0)
        }
        url = url.appendingPathComponent("home_conf.json")
        return url
    }
